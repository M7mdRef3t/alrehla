...
const ROTATING_WORDS = ["┘à┘Å┘å╪¬┘ç┘â ╪º┘ä╪¡╪»┘ê╪»", "╪¬┘å╪▓┘ü ╪╖╪º┘é╪¬┘â ╪¿╪¿╪╖╪í", "┘à┘Å╪½┘é┘ä ╪¿┘å╪¿╪╢ ╪º┘ä╪ó╪«╪▒┘è┘å", "┘ü┘è ┘à╪│╪º╪▒ ┘à╪┤╪¬╪¬"];
...
const SovereignMap: FC<{ reduceMotion: boolean | null }> = ({ reduceMotion }) => {
  ...
  const nodes = [
    { cx: 190, cy: 190 - 68,  r: 13, color: "#2dd4bf", label: "╪╣┘ä╪º┘é╪⌐ ╪¿┘à┘è╪▓╪º┘å┘ç╪º",  w: 1.2 },
    { cx: 190 + 62, cy: 190 - 34, r: 11, color: "#14b8a6", label: "╪»╪╣┘à ╪│┘è╪º╪»┘è",    w: 0.8 },
    { cx: 190 + 110, cy: 190 + 55, r: 14, color: "#f59e0b", label: "┘å╪¿╪╢ ┘à╪¬╪░╪¿╪░╪¿",  w: 1.5 },
    { cx: 190 - 60, cy: 190 + 104, r: 10, color: "#f59e0b", label: "╪¬╪┤┘ê┘è╪┤ ╪▒┘ê╪¡",   w: 0.9 },
    { cx: 190 - 130, cy: 190 - 65, r: 16, color: "#2dd4bf", label: "╪º╪¡╪¬┘ê╪º╪í ╪¡┘é┘è┘é┘è",w: 1.1 },
    { cx: 190 - 28, cy: 190 - 148, r: 12, color: "#ef4444", label: "┘å╪▓┘è┘ü ╪╖╪º┘é╪⌐",   w: 2.0 },
    { cx: 190 + 118, cy: 190 - 100, r: 11, color: "#ef4444", label: "╪¡╪»┘ê╪» ┘à┘ç╪»┘ê╪▒╪⌐", w: 1.7 },
  ];
    ...
    >
      ...
      >
        ...
        <div style={{ display: "flex", alignItems: "baseline", gap: 6 }}>
          <span style={{ fontSize: 26, fontWeight: 900, color: "#fff", fontFamily: "Tajawal" }}>┘º┘¿</span>
          <span style={{ fontSize: 11, color: "#6b8a9e", fontWeight: 700 }}>/ ┘í┘á┘á</span>
        </div>
        <div style={{
      ...
      >
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          <span style={{ fontSize: 22, fontWeight: 900, color: "#fff", fontFamily: "Tajawal" }}>┘ú</span>
          <span style={{ fontSize: 11, color: "#6b8a9e", fontWeight: 700 }}>┘à╪╡╪º╪»╪▒ ╪º┘ä╪º╪│╪¬┘å╪▓╪º┘ü</span>
        </div>
      ...
      <div style={{
        ...
        {[
          { color: "#14b8a6", label: "╪¬┘ê╪º╪▓┘å ╪░╪º╪¬┘è" },
          { color: "#f59e0b", label: "╪¬╪┤╪¬╪¬" },
          { color: "#ef4444", label: "╪º╪│╪¬┘å╪▓╪º┘ü" },
        ].map(({ color, label }) => (
...
/* ΓöÇΓöÇΓöÇ Live Pulse Counter ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ */
const PulseBadge: FC<{ count: number }> = ({ count }) => (
  ...
  >
    ...
    <span style={{ fontSize: 11, fontWeight: 800, color: "#a8bfcc", letterSpacing: "0.18em", textTransform: "uppercase" }}>
      {count.toLocaleString("ar-EG")} ┘è╪│╪¬╪╣┘è╪»┘ê┘å ┘å╪¿╪╢┘ç┘à ╪º┘ä╪ó┘å
    </span>
  </motion.div>
);
    ...
    <>
      ...
      >
        ...
        <div style={{
          ...
          {/* ΓòÉΓòÉΓòÉΓòÉ LEFT COLUMN: TEXT CONTENT ΓòÉΓòÉΓòÉΓòÉ */}
          <motion.div
            variants={stagger}
            initial="hidden"
            animate="visible"
          ...
          >
            ...
            <motion.p variants={fadeUp} className="hero-body" style={{ textAlign: "right" }}>
              ╪ú┘ê┘é┘ü ╪º┘ä╪º╪│╪¬┘å╪▓╪º┘ü ╪º┘ä╪ó┘å. ┘ü┘è ╪»┘é┘è┘é╪¬┘è┘å ┘ü┘é╪╖ ╪¬╪¬╪▒╪¼┘à ┘ç╪░┘ç ╪º┘ä┘ê╪º╪¼┘ç╪⌐
              ┘ü┘ê╪╢┘ë ╪ú┘ü┘â╪º╪▒┘â ╪Ñ┘ä┘ë ╪Ñ╪¡╪»╪º╪½┘è╪º╪¬ ┘à╪▒╪ª┘è╪⌐╪î ┘ê╪¬┘à┘å╪¡┘â ╪«╪╖┘ê╪⌐ ┘é╪º╪╖╪╣╪⌐
              ┘ä╪º╪│╪¬╪╣╪º╪»╪⌐ ╪º╪¬╪▓╪º┘å┘â.
            </motion.p>
          ...
          {/* ΓòÉΓòÉΓòÉΓòÉ RIGHT COLUMN: SOVEREIGN MAP ΓòÉΓòÉΓòÉΓòÉ */}
          <motion.div
            className="map-area"
      ...
      {/* ΓöÇΓöÇ Warp transition overlay ΓöÇΓöÇ */}
      <AnimatePresence>
        {isWarping && (
          <motion.div
