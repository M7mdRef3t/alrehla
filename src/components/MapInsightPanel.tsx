...
export const MapInsightPanel: FC = () => {
    ...
    const generateInsight = async () => {
        ...
        try {
            ...
            if (!res.ok) {
                if (res.status === 429) {
                    throw new Error('استنفذت حصتك اليومية لبصيرة الوعي (30/30). جرب تاني بكرة.');
                }
                throw new Error('فشل الاتصال بـ بصيرة الوعي');
            }
                            ...
                            <motion.div key="tab-current" initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 10 }}>
                                {loading ? (
                                    <div className="py-12 flex flex-col items-center gap-3">
                                        ...
                                        <p className="text-xs text-slate-400 animate-pulse font-medium">بصيرة الوعي بتحلل المدارات...</p>
                                    </div>
                                ) : error ? (
                                    ...
                                    <div className="py-8 text-center text-slate-500">
                                        ...
                                        <p className="text-xs">اضغط "تحليل جديد" لاستكشاف النقاط العمياء في خريطتك.</p>
                                    </div>
                                )}
                            </motion.div>
                        ) : (
                                ...
                                <div className="space-y-3">
                                    {history.length > 0 ? (
                                        ...
                                        <div className="py-8 text-center text-slate-500">
                                            ...
                                            <p className="text-xs">السجل فاضي.. استكشف بصيرتك أولاً.</p>
                                        </div>
                                    )}
                                </div>
                            </motion.div>
    ...
    <div className="space-y-6">
        <div className="flex items-center justify-between">
            ...
            {insight.id && (
                ...
                >
                    <Pin className="w-3 h-3" />
                    {insight.pinned ? 'مثبت فى السجل' : 'تثبيت الرؤية'}
                </button>
            )}
        </div>

        <div className="text-right">
            <p className="text-[11px] font-bold text-indigo-400 mb-1">الخلاصة المركزية</p>
        ...
        {insight.warning && (
            <div className="p-4 rounded-xl bg-amber-500/10 border border-amber-500/20 text-right flex items-start gap-3 justify-end group">
                <p className="text-[11px] text-amber-200 leading-relaxed">
                    <span className="font-bold text-amber-400">تحذير من لغم:</span> {insight.warning}
                </p>
...
const MiniInsightCard: FC<{ insight: MapInsight, title: string }> = ({ insight, title }) => (
    <div className="p-3 rounded-xl bg-white/5 border border-white/5 text-right">
        ...
        <div className="flex flex-wrap gap-1 justify-end">
            <span className="px-1.5 py-0.5 rounded-md bg-white/5 text-[8px] text-emerald-400">{insight.recommendations.length} مهام</span>
            <span className="px-1.5 py-0.5 rounded-md bg-white/5 text-[8px] text-blue-400">{insight.insights.length} رؤى</span>
        </div>
    </div>
);
