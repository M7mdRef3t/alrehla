import { CollapsibleSection } from '../admin/ui/CollapsibleSection';
import { logger } from "@/services/logger";
import React, { useState, useEffect } from 'react';
import { motion, AnimatePresence } from 'framer-motion';
import { Shield, CheckCircle, AlertTriangle, Eye, TrendingUp, Users, ChevronRight, Gavel, Zap, Activity, BarChart3, Globe, Database, UserPlus, Clock, Brain, Cpu } from 'lucide-react';
import { HiveEngine, ProvenPath, SwarmMetrics } from '@/services/hiveEngine';
import { fetchUsers } from "@/services/admin/adminUsers";
import { AdminUserRow, OverviewStats } from "@/services/admin/adminTypes";
...
import { PendingIntentsPanel } from './PendingIntentsPanel';
...
export const OracleCouncilDashboard: React.FC<{ oracleId: string }> = ({ oracleId }) => {
    ...
    const [activeTab, setActiveTab] = useState<'audit' | 'sovereigns' | 'vitals' | 'radar' | 'intents' | 'systems'>('audit');
    ...
    return (
        <div className="p-8 max-w-7xl mx-auto space-y-8 bg-slate-950 min-h-screen text-slate-100 font-sans" dir="rtl">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 p-8 bg-[var(--soft-teal)]/20 rounded-[2.5rem] border border-[var(--soft-teal)] backdrop-blur-3xl shadow-2xl">
                <div>
                    <h1 className="text-4xl font-black bg-clip-text text-transparent bg-gradient-to-r from-cyan-400 to-[var(--soft-teal)] flex items-center gap-4">
                        <Shield className="w-10 h-10 text-cyan-400" />
                        مجلس الأوراكل السِيادي
                    </h1>
                    <p className="mt-2 text-slate-400 uppercase tracking-widest text-xs font-bold">
                        Swarm Governance Protocol v2.0.0 (Genesis Edition)
                    </p>
                </div>
                {metrics && (
                ...
                <div className="flex-1 min-w-[300px] p-6 bg-black/40 rounded-[2rem] border border-cyan-500/10 backdrop-blur-xl relative overflow-hidden group">
                    ...
                    <div className="relative z-10 flex items-start gap-4">
                        ...
                        <div>
                            <span className="text-[9px] font-black text-cyan-500 uppercase tracking-widest block mb-1">رؤية الأوراكل</span>
                            <p className="text-xs font-medium text-slate-300 leading-relaxed italic">
                                {stats && (stats.activeConsciousnessNow || 0) > 0 
                                    ? `الوعي الجماعي في حالة "تدفق". ${(stats.totalTravelers || 0) > 100 ? 'التوسع يتطلب تعزيز بروتوكولات الحماية.' : 'المسار آمن للتوسع التدريجي.'}`
                                    : "السكون يسود الشبكة. وقت مثالي لمراجعة المسارات المعلقة."}
                            </p>
                        </div>
                    </div>
                </div>
            ...
            <div className="flex flex-wrap gap-4 p-2 bg-slate-900/60 rounded-[2rem] border border-white/5 w-fit backdrop-blur-md shadow-inner">
                ...
                <button
                    ...
                    className={`px-8 py-3 rounded-2xl flex items-center gap-2 text-xs font-black uppercase tracking-widest transition-all duration-500 ${activeTab === 'intents' ? 'bg-gradient-to-r from-orange-500 to-rose-600 text-white shadow-lg shadow-orange-500/20' : 'text-slate-500 hover:text-white'}`}
                >
                ...
                <button
                    ...
                    className={`px-8 py-3 rounded-2xl flex items-center gap-2 text-xs font-black uppercase tracking-widest transition-all duration-500 ${activeTab === 'systems' ? 'bg-gradient-to-r from-slate-700 to-slate-900 text-white shadow-lg shadow-white/5 border border-white/10' : 'text-slate-500 hover:text-white'}`}
                >
            ...
            <AnimatePresence mode="wait">
                    ...
                    >
                        ...
                        <div className="lg:col-span-5">
                            <CollapsibleSection
                                ...
                                subtitle="Pending Audit Queue"
                                icon={<Gavel className="w-4 h-4" />}
                                defaultExpanded={true}
                                badge={<span className="text-[10px] font-black px-2 py-0.5 bg-cyan-500/20 text-cyan-400 rounded-md border border-cyan-500/30">{pending.length}</span>}
                            >
                                <div className="space-y-4 max-h-[600px] overflow-y-auto pr-3 custom-scrollbar mt-4">
                                    {pending.map((path) => (
                                        <motion.div
                                            key={path.id}
                                            layout
                        ...
                        <div className="lg:col-span-7">
                            <CollapsibleSection
                                title="تفاصيل المسار"
                                subtitle="Path Visualizer & Decision Core"
                ...
                {activeTab === 'sovereigns' && (
                    ...
                    >
                        <CollapsibleSection
                            title="رفاق الرحلة النشطين"
                            subtitle={`Active Sovereigns List (${users.length})`}
                            icon={<Users className="w-4 h-4" />}
                            defaultExpanded={true}
                            badge={
                                <button onClick={() => window.location.reload()} className="p-1 bg-white/5 hover:bg-white/10 rounded-lg border border-white/5 transition-all text-emerald-400">
                ...
                {activeTab === 'vitals' && (
                    ...
                    >
                        <CollapsibleSection
                            title="نبض الوعي الجماعي"
                            subtitle="Collective Consciousness Vitals"
                        ...
                        <CollapsibleSection
                            title="تحليلات النمو والوعي"
                            subtitle="Growth Matrix & System Awareness"
                            icon={<TrendingUp className="w-4 h-4" />}
                            defaultExpanded={true}
                        >
                            <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 pt-6 mt-4">
                ...
                {activeTab === 'radar' && (
                    ...
                    >
                        <CollapsibleSection
                            title="رادار السلوك"
                            subtitle="Behavioral Resonance Radar"
                ...
                {activeTab === 'intents' && (
                    ...
                    >
                        <CollapsibleSection
                            title="نوايا العبور"
                            subtitle="Sovereign Transition Intents"
                ...
                {activeTab === 'systems' && (
                    ...
                    >
                        <CollapsibleSection
                            title="الأنظمة والنبض"
                            subtitle="Live Telemetry & Hive Status"
