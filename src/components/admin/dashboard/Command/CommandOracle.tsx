...
import { Zap as Sparkles, Zap, Brain, Eye, Terminal, Loader2, RefreshCw, Activity, ShieldCheck, ChevronDown, ChevronUp, Copy, Check, MessageSquare, Send } from "lucide-react";
import { fetchCommandInsights, respondToOracleInsight } from "@/services/admin/adminAlerts";
...
export const CommandOracle: FC<CommandOracleProps> = ({ minimal }) => {
  const { commandInsights, setCommandInsights, commandStats, setCommandStats } = useAdminState();
  ...
  const refreshInsights = useCallback(async () => {
    ...
    try {
      const result = await fetchCommandInsights();
      if (result) {
        if (result.insights && result.insights.length > 0) {
          ...
          setCommandInsights(localizedInsights);
          ...
          if (validAugmentations.length > 0) {
            const currentInsights = useAdminState.getState().commandInsights;
            const updatedInsights = currentInsights.map(item => {
              const aug = validAugmentations.find(a => a.id === item.id);
              return aug ? { ...item, ...aug } : item;
            });
            setCommandInsights(updatedInsights);
          }
        }
        if (result.stats) {
          setCommandStats(result.stats);
        }
  ...
  }, [setCommandInsights, setCommandStats, cooldown]);
  ...
  }, [commandInsights.length, refreshInsights]);

  const sortedFriction = commandStats?.behavioralFriction?.filter(f => f.avgTimeSec > 30) || [];
        ...
        <div className="p-4 border-b border-white/5 bg-white/10 backdrop-blur-md flex items-center justify-between flex-row-reverse sticky top-0 z-20">
          <div className="flex items-center gap-3 text-indigo-400 flex-row-reverse text-right">
            ...
            <div>
              ...
              <p className="text-[9px] opacity-40 font-mono tracking-tighter uppercase">Command Intelligence Pulse</p>
            </div>
          </div>
      ...
      <div className={`flex-1 overflow-y-auto ${minimal ? '' : 'p-4'} space-y-4 custom-scrollbar relative`}>
        ...
        <AnimatePresence mode="popLayout" initial={false}>
          {commandInsights.length > 0 ? (
            commandInsights.map((insight: any, idx: number) => {
