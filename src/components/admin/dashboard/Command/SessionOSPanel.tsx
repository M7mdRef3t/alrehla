...
interface Session {
  ...
  client_name: string;
  client_phone?: string;
  user_id?: string; // New: Command link
  is_Command_captured?: boolean; // New: Sync flag
...
export const SessionOSPanel: React.FC = () => {
  ...
  const [CommandProfile, setCommandProfile] = useState<{ diagnosis?: string; interpretation?: string } | null>(null);
  const [isFetchingCommand, setIsFetchingCommand] = useState(false);
  ...
  useEffect(() => {
    if (selected) {
      fetchCommandData(selected);
    } else {
      setCommandProfile(null);
    }
  }, [selected?.id]);

  const fetchCommandData = async (session: Session) => {
    if (!isSupabaseReady || !supabase) return;
    setIsFetchingCommand(true);
    ...
    try {
      ...
      console.error("Failed to fetch Command data:", e);
    } finally {
      setIsFetchingCommand(false);
    }
  };
  ...
  const triggerWhatsApp = async (session: Session) => {
    ...
    try {
      const res = await fetch("/api/admin/Command/whatsapp", {
  ...
  const triggerMasarat = async (session: Session) => {
    ...
    try {
      const res = await fetch("/api/admin/Command/masarat", {
        method: "POST", headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ sessionId: session.id })
      });
      const data = await res.json();
      ...
      const isSynced = data.is_Command_synced;
      
      const updated = sessions.map(s => s.id === session.id ? { 
        ...s, 
        ai_summary: summary, 
        status: "active" as SessionStatus,
        is_Command_captured: isSynced 
      } : s);
                        ...
                        <div className="flex items-center gap-2 min-w-0">
                          ...
                          {session.is_Command_captured && (
                            <div className="flex items-center gap-1 px-1.5 py-0.5 rounded-md bg-cyan-500/10 border border-cyan-500/20 text-cyan-400 group-hover:bg-cyan-500/20 transition-all shadow-[0_0_8px_rgba(6,182,212,0.15)]">
                              ...
                              <span className="text-[7px] font-black uppercase tracking-tighter">Command Link</span>
                            </div>
                          )}
                        </div>
        ...
        <AnimatePresence>
          {selected && (
            ...
            >
              ...
              {/* Command Insights */}
              <AnimatePresence>
                {(CommandProfile || isFetchingCommand) && (
                  <motion.div
                    ...
                    <div className="p-4 bg-cyan-500/5 border border-cyan-500/10 rounded-2xl space-y-3 relative overflow-hidden group">
                      ...
                      {isFetchingCommand ? (
                        ...
                        <div className="space-y-4">
                          {CommandProfile?.diagnosis && (
                            <div className="space-y-1">
                              <p className="text-[9px] font-black text-slate-500 uppercase">التشخيص الحالي</p>
                              <p className="text-xs text-slate-300 leading-relaxed">{CommandProfile.diagnosis}</p>
                            </div>
                          )}
                          {CommandProfile?.interpretation && (
                            <div className="space-y-1">
