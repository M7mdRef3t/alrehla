...
import { GovernanceHistoryDNA } from "./GovernanceHistoryDNA";
...
export const GovernanceHub: FC = () => {
  ...
  const [viewMode, setViewMode] = useState<"list" | "dna">("dna");
        ...
        <div className="relative z-10 flex items-center gap-6">
          ...
          <div>
            <div className="flex items-center gap-3 mb-1">
               ...
               <span className="px-3 py-1 bg-teal-500/10 border border-teal-500/30 rounded-full text-[10px] font-black text-teal-400 uppercase tracking-widest">Command Alpha</span>
            </div>
        ...
        <div className="lg:col-span-3 space-y-4">
           <div className="p-6 rounded-3xl bg-[#0B0F19] border border-white/5 space-y-6 shadow-xl">
             ...
             <div className="pt-4 space-y-3">
               <h3 className="text-[10px] font-black text-slate-500 uppercase tracking-[0.2em] px-2 mb-2">نمط العرض</h3>
               <div className="flex p-1 bg-slate-950 border border-white/5 rounded-xl">
                  <button 
                    onClick={() => setViewMode("list")}
                    className={`flex-1 py-2 text-[9px] font-black uppercase tracking-widest rounded-lg transition-all ${viewMode === 'list' ? 'bg-slate-800 text-white shadow-lg' : 'text-slate-500 hover:text-slate-400'}`}
                  >
                    قائمة (List)
                  </button>
                  <button 
                    onClick={() => setViewMode("dna")}
                    className={`flex-1 py-2 text-[9px] font-black uppercase tracking-widest rounded-lg transition-all ${viewMode === 'dna' ? 'bg-teal-500 text-white shadow-lg shadow-teal-500/20' : 'text-slate-500 hover:text-slate-400'}`}
                  >
                    سلسلة (DNA)
                  </button>
               </div>
           ...
           <div className={`p-8 rounded-3xl ${aiAutonomyEnabled ? 'bg-gradient-to-br from-teal-900/40 to-slate-900 border-teal-500/20' : 'bg-gradient-to-br from-indigo-900/40 to-slate-900 border-indigo-500/20'} shadow-inner relative overflow-hidden group transition-colors duration-500`}>
              ...
              <p className={`text-xs ${aiAutonomyEnabled ? 'text-teal-300' : 'text-indigo-300'} font-bold tracking-wide leading-relaxed relative z-10 mb-6`}>
                {aiAutonomyEnabled 
                  ...
                  : "النظام الآن يعمل تحت القيادة البشرية. القرارات المالية والاستراتيجية تتطلب موافقتك."}
              </p>
              ...
              >
                {aiAutonomyEnabled ? "تعليق الاستقلالية" : "تمكين القيادة المطلقة"}
              </button>
           </div>
        </div>
        ...
        <div className="lg:col-span-9 space-y-4">
           {viewMode === "dna" ? (
             <div className="bg-slate-950/40 rounded-[2.5rem] border border-white/5 overflow-hidden">
               <GovernanceHistoryDNA decisions={filteredDecisions} />
             </div>
