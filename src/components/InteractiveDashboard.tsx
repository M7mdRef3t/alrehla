...
const MOODS = [
  ...
  { v: 2, emoji: "😔", label: "متعب", color: "#FBBF24" },
  { v: 3, emoji: "😐", label: "عادي", color: "#94a3b8" },
  { v: 4, emoji: "🙂", label: "بخير", color: "#34D399" },
  { v: 5, emoji: "✨", label: "رائع", color: "#14B8A6" },
];
...
function getGreeting() {
  ...
  if (h < 6) return { text: "ليلة هادئة 🌙", color: "#A78BFA" };
  if (h < 12) return { text: "صباح النور ☀️", color: "#FBBF24" };
...
function HeroSection({ xp, level, moodToday }: { xp: number; level: number; moodToday: PulseEntry | null }) {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }}
      style={{
        background: "linear-gradient(135deg, rgba(20,184,166,0.08) 0%, rgba(167,139,250,0.06) 100%)",
        border: "1px solid rgba(20,184,166,0.15)", borderRadius: 22,
      ...
      <div style={{ marginTop: 14 }}>
        ...
        <div style={{ height: 5, borderRadius: 3, background: "rgba(255,255,255,0.05)" }}>
          <motion.div
            ...
            style={{ height: "100%", background: "linear-gradient(90deg, #14B8A6, #0d9488)", borderRadius: 3 }}
          />
        </div>
      </div>
    </motion.div>
...
function DashboardSidebar({
  ...
  return (
    <aside style={{
      width: 200, flexShrink: 0, background: "rgba(255,255,255,0.02)",
      ...
      <div style={{ marginBottom: 18, paddingRight: 4 }}>
        <p style={{ margin: 0, fontSize: 13, fontWeight: 900, color: "#14B8A6" }}>
          Insight Panel
        </p>
        <p style={{ margin: "1px 0 0", fontSize: 8, color: "#334155", textTransform: "uppercase", letterSpacing: 1 }}>
        ...
        return (
          <button key={item.id} onClick={() => onChange(item.id)}
            style={{
              ...
              border: `1px solid ${isActive ? "rgba(20,184,166,0.25)" : "transparent"}`,
...
export function InteractiveDashboard({ onBack }: InteractiveDashboardProps) {
  ...
  return (
    <div dir="rtl" style={{ minHeight: "100vh", background: "#080b15", padding: "20px 16px 80px" }}>
