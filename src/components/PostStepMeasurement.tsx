...
export const PostStepMeasurement: FC<PostStepMeasurementProps> = ({ onComplete }) => {
  ...
  const handleNext = () => {
    if (isLast) {
      const score = computePostStepScore(answers);
      completePostStep(answers, score);
      onComplete();
      return;
    }
  ...
  return (
    <div className="w-full max-w-lg mx-auto text-center">
      <p className="text-slate-600 mb-6">قبل ما نخلص، نقيس مع بعض التغيير اللي حصل.</p>
      <div className="mb-6">
        <span className="text-sm text-slate-500">
          {currentIndex + 1} / {POST_STEP_QUESTIONS.length}
        </span>
        <div className="mt-2 h-1.5 bg-slate-200 rounded-full overflow-hidden">
          <motion.div
            className="h-full bg-teal-500 rounded-full"
            initial={{ width: 0 }}
            animate={{ width: `${((currentIndex + 1) / POST_STEP_QUESTIONS.length) * 100}%` }}
