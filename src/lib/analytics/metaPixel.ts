...
export const pageview = () => {
  if (typeof window !== 'undefined' && (window as unknown as any).fbq) {
    (window as unknown as any).fbq('track', 'PageView');
  }
};
...
export const event = (name: string, options: any = {}, eventId?: string) => {
  if (typeof window !== 'undefined' && (window as unknown as any).fbq) {
    ...
    (window as unknown as any).fbq('track', name, options, trackingParams);
  } else {
...
export const customEvent = (name: string, options: any = {}, eventId?: string) => {
  if (typeof window !== 'undefined' && (window as unknown as any).fbq) {
    const trackingParams = eventId ? { eventID: eventId } : undefined;
    (window as unknown as any).fbq('trackCustom', name, options, trackingParams);
  } else {
