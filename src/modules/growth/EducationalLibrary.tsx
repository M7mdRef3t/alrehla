...
import { X, Video, BookOpen, HelpCircle, Search, Tag, ArrowLeft } from "lucide-react";
...
const VideoCard: FC<{ video: VideoContent }> = ({ video }) => {
  ...
  useEffect(() => {
    ...
    trackAffiliateLinkExposed(video.videoUrl, {
      placement: "educational_library_video",
      contentId: video.id,
      title: video.title
    });
  ...
  const handleVideoLinkClick = () => {
    if (!video.videoUrl) return;
    trackAffiliateLinkClicked(video.videoUrl, {
      placement: "educational_library_video",
      contentId: video.id,
      title: video.title
    });
