...
import { Step1View } from '@/modules/action/PulseCheckModalParts/Step1View';
import { Step2View } from '@/modules/action/PulseCheckModalParts/Step2View';
import { PhoneCaptureView } from '@/modules/action/PulseCheckModalParts/PhoneCaptureView';
import { getStoredLeadPhone, captureMarketingLead } from "@/services/marketingLeadService";
import { AnalysisOverlay } from '@/modules/action/PulseCheckModalParts/AnalysisOverlay';
import { WarpVelocityEffect } from '@/modules/action/PulseCheckModalParts/WarpVelocityEffect';
...
export const PulseCheckModal: FC<PulseCheckModalProps> = ({
  ...
  // Emotional Background Logic
  const moodNebula = useMemo(() => {
    if (!mood) return "";
    return MOOD_COSMIC[mood].nebula;
  }, [mood]);
      ...
      {isOpen && (
        ...
        >
          <div
            ...
            style={{
              background: `${moodNebula ? `${moodNebula}, ` : ""}${energyGradient(energy)}, radial-gradient(ellipse at 20% 30%, rgba(139, 92, 246, 0.1) 0%, transparent 60%), radial-gradient(ellipse at 80% 70%, rgba(45, 212, 191, 0.08) 0%, transparent 55%), var(--ds-color-space-void)`,
