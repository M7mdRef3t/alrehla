...
export const SymptomsChecklist: FC<SymptomsChecklistProps> = ({
  ...
  const testOptions: { value: TestAnswer; label: string; color: string }[] = [
    { value: "green", label: "???? / ???????", color: "bg-teal-100 border-teal-400 text-teal-800" },
    { value: "yellow", label: "??? / ???", color: "bg-amber-100 border-amber-400 text-amber-800" },
    { value: "red", label: "??? / ???? / ??????", color: "bg-rose-100 border-rose-400 text-rose-800" }
  ];
  ...
  const testResult: TestAnswer | null = testComplete
    ...
    : null;
  const testResultLabel = testResult === "green" ? "?? ???? / ???????" : testResult === "yellow" ? "?? ??? / ???" : testResult === "red" ? "?? ??? / ???? / ??????" : null;
  ...
  const dominantCategories = (Object.keys(groupedSymptoms) as SymptomCategory[])
    ...
    .slice(0, 2)
    .map((x) => x.category);
  const dominantLabels = dominantCategories.map(getCategoryLabel).join(" ? ");
  ...
  const getImpactCopy = () => {
    ...
    const levelText = impactLevel === "light" ? "????" : impactLevel === "medium" ? "?????" : "????";
      ...
      {impactCopy && selectedCount > 0 && (
        ...
        >
          <div className="flex flex-wrap items-center justify-end gap-2 mb-2">
            <span className="text-xs font-bold px-2.5 py-1 rounded-full bg-white/80">???? ???????: {impactCopy.levelText}</span>
            {impactCopy.relationshipLabel && <span className="text-xs font-semibold">???? ??????: {impactCopy.relationshipLabel}</span>}
            {dominantLabels && <span className="text-xs font-semibold">??????: {dominantLabels}</span>}
            <span className="text-xs text-slate-600">????: {totalWeight}</span>
          </div>
