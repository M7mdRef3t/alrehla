...
const MOCK_ORACLE_INSIGHTS = [
    { risk: 85, tip: "العميل في منطقة خطر. لوحظ نمط متكرر من إهمال الذات في الدائرة العملية. يُنصح بالتدخل الفوري لمراجعة المهام اليومية.", trend: "up" },
    { risk: 40, tip: "مؤشرات مبكرة على إرهاق من ضغوط عائلية. تم تفعيل 'خطة الانسحاب التكتيكي' بنجاح أمس.", trend: "stable" },
    { risk: 15, tip: "مرحلة استقرار ممتازة. العميل يحافظ على مسافات صحية مع الأشخاص في الدائرة الحمراء.", trend: "down" },
];
...
export default function CoachDashboard() {
    ...
    if (isLoading) {
        return <div className="min-h-screen bg-white flex items-center justify-center font-bold text-slate-400">تحميل لوحة التجكم...</div>;
    }
        ...
        return (
            <div className="min-h-screen bg-[#f8fafc] flex flex-col items-center justify-center p-6 text-center" dir="rtl">
                ...
                <p className="text-gray-600 max-w-md leading-relaxed mb-8">
                    تحتاج لرخصة "المدرب" للوصول للوحة التحكم وإدارة عملائك والاطلاع على رادار الحالات.
                </p>
                <div className="flex gap-4">
                    <a href="/" className="px-8 py-4 bg-white text-gray-900 border border-gray-200 rounded-2xl font-bold hover:bg-gray-50 transition">العودة للرئيسية</a>
                    <a href="/pricing" className="px-8 py-4 bg-gray-900 text-white rounded-2xl font-bold hover:bg-black transition shadow-lg shadow-gray-900/20">ترقية لرخصة مدرب</a>
                </div>
            </div>
        );
    }
            ...
            <aside className="w-72 bg-white border-l border-slate-200 flex flex-col shrink-0 relative z-20 shadow-sm">
                ...
                <nav className="flex-1 px-4 py-6 flex flex-col gap-2">
                    ...
                    >
                        ...
                        <span className="flex-1 text-right">رادار التنبيهات</span>
                        {alerts.length > 0 && (
                    ...
                    <button className="flex items-center gap-3 w-full p-3.5 rounded-xl text-slate-500 hover:bg-slate-50 hover:text-slate-700 font-bold transition-colors">
                        ...
                        <span className="flex-1 text-right">إدارة الجلسات</span>
                    </button>
                </nav>
                ...
                <div className="p-4 border-t border-slate-100 bg-slate-50/50">
                    <div className="flex items-center gap-3 mb-4 px-2">
                        ...
                        <div className="min-w-0">
                            <div className="truncate text-sm font-bold text-slate-900">{coach.user_metadata?.full_name || 'أخصائي معتمد'}</div>
                    ...
                    {selectedClient ? (
                        ...
                        >
                                ...
                                <div className="flex justify-between items-center w-full mb-6">
                                    <div className="flex items-center gap-4">
                                        ...
                                        <div>
                                            ...
                                            <p className="text-sm font-medium text-slate-500 mt-1">تحديث الخريطة: قبل ساعتين ({selectedClient.lastUpdate})</p>
                                        </div>
                                    </div>
                                    <div className="flex items-center gap-3">
                            ...
                            <div className="flex-1 p-8 overflow-y-auto">
                                <div className="bg-slate-200/50 border-2 border-dashed border-slate-300 rounded-3xl h-full min-h-[500px] flex flex-col items-center justify-center text-slate-500">
                                    ...
                                    <p className="font-bold text-lg">منظور المعالج (Therapist View)</p>
                                    <p className="text-sm mt-2 max-w-sm text-center">جاري الربط مع محرك React Flow لعرض خريطة "دواير" الخاصة بالعميل مع إخفاء الأسماء الحقيقية للملفات (Privacy-First).</p>
                                </div>
                            </div>
                        </motion.div>
                    ) : (
                        ...
                        >
                            <header className="mb-10 flex justify-between items-end">
                                <div>
                                    <h2 className="text-3xl font-black text-slate-900 mb-2">رادار الحالات</h2>
                                    <p className="text-slate-500 font-medium text-lg">نظرة شاملة على مستويات الاستنزاف والطاقة لعملائك النشطين.</p>
                                </div>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10">
                                <div className="bg-white p-6 rounded-3xl shadow-sm border border-slate-100 flex items-center justify-between">
                                    <div>
                                        <div className="text-sm font-bold text-slate-400 uppercase tracking-widest mb-1">العملاء النشطين</div>
                                ...
                                <div className="bg-gradient-to-br from-rose-500 to-red-600 p-6 rounded-3xl shadow-md border border-red-500 flex items-center justify-between text-white relative overflow-hidden">
                                    ...
                                    <div className="relative z-10">
                                        <div className="text-sm font-bold text-rose-100 uppercase tracking-widest mb-1">حالات تتطلب تدخل (Triage)</div>
                                ...
                                <div className="bg-white p-6 rounded-3xl shadow-sm border border-slate-100 flex items-center justify-between">
                                    <div>
                                        <div className="text-sm font-bold text-slate-400 uppercase tracking-widest mb-1">تحديثات اليوم</div>
                            ...
                            <div className="bg-white rounded-3xl shadow-sm border border-slate-200 overflow-hidden">
                                ...
                                <div className="divide-y divide-slate-100">
                                    {filteredClients.length === 0 ? (
                                        <div className="p-12 text-center text-slate-500 font-medium">لا يوجد عملاء يطابقون بحثك.</div>
                                                ...
                                                <div className="flex items-center gap-3 mb-1.5">
                                                    ...
                                                    {client.burnoutProbability >= 70 && (
                                                        <span className="flex items-center gap-1 px-2 py-0.5 bg-rose-100 text-rose-700 rounded text-[10px] font-bold uppercase tracking-widest border border-rose-200">
                                                            <div className="w-1.5 h-1.5 rounded-full bg-rose-500 animate-pulse" /> خطر
                                                        </span>
                                                    )}
                                                </div>
                                                <div className="flex items-center gap-4 text-sm text-slate-500 font-medium">
                                                    <span className="flex items-center gap-1.5">
                                                        <Clock className="w-4 h-4" /> حدثت الخريطة: {client.lastUpdate}
                                                    </span>
                                            ...
                                            {client.status === 'active' && (
                                                <div className="flex items-center gap-8 shrink-0">
                                                    <div className="flex flex-col items-end">
                                                        <span className="text-[11px] font-bold text-slate-400 uppercase tracking-widest mb-1">مؤشر الاستنزاف</span>
                                                        <div className="flex items-center gap-3">
