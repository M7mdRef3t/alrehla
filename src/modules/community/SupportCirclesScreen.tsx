...
export default function SupportCirclesScreen() {
    ...
    if (isLoading) {
        return (
            <div className="w-full min-h-[100dvh] flex items-center justify-center" style={{ background: "linear-gradient(160deg, #0f172a 0%, #1e1b4b 50%, #0f172a 100%)" }}>
                <div className="w-8 h-8 rounded-full border-4 border-[var(--color-primary)] border-t-indigo-500 animate-spin" />
            </div>
        );
    }
    ...
    return (
        ...
        >
            {/* Header */}
            <header className="px-6 py-8 border-b border-[var(--color-primary)] bg-black/20 backdrop-blur-md sticky top-0 z-20">
                <div className="flex items-center gap-3 mb-2">
                    ...
                    <div>
                        ...
                        <p className="text-xs text-[var(--color-primary)] font-medium">مساحة آمنة جداً. بلا أسماء. بلا أحكام.</p>
                    </div>
                </div>

                <div className="flex gap-2 mt-6 p-1 rounded-xl bg-[var(--color-primary)]/40 border border-[var(--color-primary)]">
                    <button
                        onClick={() => setActiveTab('circles')}
                        className={`flex-1 py-2 text-sm font-bold rounded-lg transition-all ${activeTab === 'circles' ? 'bg-[var(--color-primary)] text-white shadow-lg shadow-[var(--color-primary-glow)]' : 'text-slate-400 hover:text-white'}`}
                    >
                        الدوائر الحية
                    </button>
                    <button
                        onClick={() => setActiveTab('wisdom')}
                        className={`flex-1 py-2 text-sm font-bold rounded-lg transition-all ${activeTab === 'wisdom' ? 'bg-[var(--color-primary)] text-white shadow-lg shadow-[var(--color-primary-glow)]' : 'text-slate-400 hover:text-white'}`}
                    >
                        حكمة الشارع
                    </button>
                </div>
            ...
            <main className="p-6">
                <AnimatePresence mode="wait">
                    {activeTab === 'circles' ? (
                        ...
                        >
                            {/* Call to action card */}
                            <div className="p-5 rounded-2xl bg-[var(--color-primary)]/10 border border-[var(--color-primary)] flex flex-col gap-3 relative overflow-hidden">
                                <div className="absolute top-0 right-0 w-32 h-32 bg-[var(--color-primary)]/10 rounded-full blur-2xl -mt-10 -mr-10" />
                                <Lock className="w-6 h-6 text-[var(--color-primary)] mb-1" />
...
const CircleCard = ({ circle }: { circle: SupportCircle }) => {
    return (
        <div className="p-5 rounded-2xl bg-slate-800/40 border border-slate-700 hover:border-[var(--color-primary)] transition-colors flex flex-col gap-4">
            ...
            <button className="w-full py-3 rounded-xl bg-[var(--color-primary)] font-bold text-xs text-white hover:bg-[var(--color-primary)] transition-colors shadow-lg shadow-[var(--color-primary-glow)] flex items-center justify-center gap-2">
