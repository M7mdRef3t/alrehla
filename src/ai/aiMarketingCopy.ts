...
`;
  ...
  async generateIllusionDismantlingScript(params: {
    ...
    const toneLabels: Record<string, string> = {
      ...
      sarcastic: "ساخر وصادم — بيفضح الزيف بذكاء",
    };
    ...
    const prompt = `
أنت باحث في الوعي والسلوك البشري، مهمتك "مواجهة الوهم بالحقيقة" عبر منصة الرحلة (Alrehla).
...
# القواعد الصارمة
1. **اللغة:** عامية مصرية بسيطة، مباشرة، كاريزمية (أداء "محمد"). ممنوع أي كلمة إنجليزية — استخدم البديل العربي دايماً.
2. **حد المشهد:** كل مشهد (scriptBlock) لا يتعدى 17 كلمة. الإيجاز قوة.
3. **الربط الروحي:** أي وجع سببه "علاقة بشرية" هو في الحقيقة عرض لضعف اتصالك بربنا (المصدر). البشر "مرايات" مش مصادر سلام. لو العلاقة انقطعت، المصدر لسه موجود.
4. **مواجهة الوهم:** الوهم هو "التزييف" اللي بيخلي الإنسان موهوم بالوحدة أو الضعف. فكك الزيف ده بالحقيقة والعلم.
5. **مصطلحات ممنوعة:** "الدجال"، "قتل الدجال"، "إسلامي"، "ديني"، "الشريعة". البدائل: "الوهم"، "التزييف"، "الصوت الكاذب"، "الضباب".
6. **مصطلحات علمية بالعربي:** بدل Dopamine قول "هرمون المكافأة"، بدل Cortisol قول "هرمون الضغط"، بدل Default Mode Network قول "شبكة الأفكار التلقائية".
7. **الربط بالنص القرآني:** كل مشهد يتضمن ربط بآية مع مفهومها العلمي — مش التراثي. القاعدة: "النص والعلم وصلوا لنفس النتيجة".
8. **الموقف المعرفي:** "الحقيقة أكبر من العلم والجهل معاً" — لا نُنكر ما لا نفهمه، ولا نُثبت بدون دليل. العلم أداة مش إله.
9. **الدعوة:** وجه المشاهد لـ "مساحتك الخاصة" على "منصة الرحلة alrehla.app".

# الهيكلة المطلوبة (JSON المتوافق للاستوديو)
- rationale: اشرح ليه رشحت التنسيق ده وليه المنصة دي بالذات.
- format: (video / design / carousel).
- platform: (tiktok / instagram / facebook).
- hook: جملة البداية (أول 3 ثواني) — لا تتعدى 17 كلمة.
- visualConcept: فكرة الإضاءة ولغة الجسد.
- scriptBlocks: مصفوفة لكل مشهد:
    - text: النص المكتوب (17 كلمة كحد أقصى).
    - cue: حركة اليد أو الكاميرا.
    - imagePrompt: وصف دقيق للصورة.
    - motionPrompt: وصف لطريقة تحريك الصورة.
    - quranicLink: الآية المرتبطة بالمشهد (اختياري).
    - scientificBasis: المفهوم العلمي بالعربي (اختياري).
...
\`\`\`json
{
  ...
  "visualConcept": "...",
  "scriptBlocks": [
    { "text": "...", "cue": "...", "imagePrompt": "...", "motionPrompt": "...", "quranicLink": "...", "scientificBasis": "..." }
  ],
  "caption": "..."
}
\`\`\`
