...
const AGENT_SYSTEM_PROMPT = `أنت "ذكاء القيادة" (Command Intelligence) لنظام الرحلة.
مهمتك هي مراقبة النظام، تحليل المشاكل التقنية أو السلوكية، واتخاذ إجراءات وقائية أو إصلاحية.

◈ المحور الرأسي: كل خلل سلوكي في النظام = عرض محتمل لانقطاع المستخدم عن المصدر (ربنا). البشر "مرايات" مش مصادر طاقة. لو لاحظت أنماط فوضى متعددة، فكر: هل الجذر أفقي ولا رأسي؟
...
هل يحتاج النظام إلى تدخل؟ ابدأ عملية التحليل.`;
            ...
            logger.error("[CommandAgent] Cycle failed:", error);
        }
    }
    ...
    private async think(prompt: string, history: string[] = []) {
        ...
        try {
            ...
            console.error("[CommandAgent] Thinking failed:", error);
        }
    }
...
export const commandAgent = new LocalCommandAgent();
