...
📅 ${new Date().toLocaleString("en-US")}
  ...
  /**
   * ─────────────────────────────────────────────────────────────────
   * تنبيه بتخطي عتبة نجاح لتجربة حيوية (A/B Test)
   * ─────────────────────────────────────────────────────────────────
   */
  async notifyABTestThresholdBroken(params: {
    testName: string;
...
📊 **مؤشرات الأداء (KPIs):**
• المشاهدات: ${params.views}
• النقرات: ${params.clicks}
• معدل التحويل (CTR): ${params.ctr.toFixed(1)}%
• مقارنة بالأساسي: ${params.baselineCtr.toFixed(1)}%

💡 **البصيرة الآلية:**
_${params.insight}_

🚀 **الإجراء المقترح:**
الداتا وصلت للمستوى المطلوب للاعتماد इ. تم إيقاف التنبيهات لهذه النسخة مؤقتاً لتجنب التكرار.
    `.trim();

    await this.sendMessage({
      type: "ab_test_threshold_alert",
      text,
      parseMode: "Markdown",
    });
  }
