...
function isUnstableMood(mood: PulseEntry["mood"]): boolean {
...
function signalWeight(signal: DawayirSignalEventV1): number {
  switch (signal.type) {
    case "ring_changed":
      return 2.2;
...
function journeyEventWeight(event: JourneyEvent): number {
...
function pulseWeight(entry: PulseEntry): number {
...
function computePulseInstability(pulses: PulseEntry[], now: number): number {
  const recent = pulses
...
function computeTaskCompletion(events: JourneyEvent[]): number {
...
function computeShadowIntensity(shadowScores: Record<string, { score: number }>): number {
  ...
  if (values.length === 0) return 0;
  return clamp(average(values.map((score) => score.score)) / 100, 0, 1);
}

function ringRatios(nodes: MapNode[]): {
  red: number;
  yellow: number;
  detached: number;
} {
  const active = nodes.filter((node) => !node.isNodeArchived);
  if (active.length === 0) return { red: 0, yellow: 0, detached: 0 };
  const red = active.filter((node) => node.ring === "red").length / active.length;
  const yellow = active.filter((node) => node.ring === "yellow").length / active.length;
  const detached = active.filter((node) => node.detachmentMode || node.isDetached).length / active.length;
  return { red, yellow, detached };
}

function computeRuminationNorm(nodes: MapNode[]): number {
  const active = nodes.filter((node) => !node.isNodeArchived);
  if (active.length === 0) return 0;
  const total = active.reduce((sum, node) => sum + (node.recoveryProgress?.ruminationLogCount ?? 0), 0);
  return clamp(total / (active.length * 8), 0, 1);
}

function computeBoundaryMean(nodes: MapNode[]): number {
  const values = nodes
    .filter((node) => !node.isNodeArchived)
    .map((node) => node.recoveryProgress?.boundaryLegitimacyScore)
    .filter((score): score is number => typeof score === "number");
  if (values.length === 0) return 0;
  return clamp(average(values) / 100, 0, 1);
}

function computeStageMaturity(nodes: MapNode[]): number {
  const stages = nodes
    .filter((node) => !node.isNodeArchived)
    .map((node) => normalizePathStage(node.recoveryProgress?.pathStage));
  if (stages.length === 0) return 0;
  return average(stages.map(stageScore));
}

function computeCompletionRatio(nodes: MapNode[]): number {
  const active = nodes.filter((node) => !node.isNodeArchived);
  if (active.length === 0) return 0;
...
function computeMissionCompletion(nodes: MapNode[]): number {
  const active = nodes.filter((node) => !node.isNodeArchived);
  if (active.length === 0) return 0;
  const completed = active.filter((node) => node.missionProgress?.isCompleted === true).length;
...
export interface RelationshipFlowVector {
  inflow: number; // 0..1
  outflow: number; // 0..1
  netFlux: number; // -1..1 (positive = pressure rising)
  stability: number; // 0..1
  oscillationIndex: number; // 0..1
  dominantFrequencyHours: number | null;
}

export interface PainFieldMetrics {
  painFieldIntensity: number; // 0..100
  baselinePain: number; // 0..100
  recoveryMomentum: number; // -100..100
  painDividend: number; // 0..100
}

export type TwinScenarioId =
  | "no_action"
  | "micro_regulation"
  | "soft_boundary"
  | "targeted_reflection"
...
function detectHiddenPattern(
  ...
  for (let lag = 2; lag <= 12; lag += 1) {
    ...
    // Keep correlation strong, but slightly prefer shorter cycles for actionability.
...
function computePainMetrics(input: {
  nodes: MapNode[];
  pulseInstability: number;
  hesitation: number;
  shadowIntensity: number;
  ...
  const volatility = clamp(
    input.signals.filter((signal) => signal.type === "ring_changed").length /
      Math.max(4, input.nodes.filter((node) => !node.isNodeArchived).length * 2),
    0,
    1
  );
...
function buildTwinDecision(input: {
  pain: PainFieldMetrics;
  flow: RelationshipFlowVector;
  hiddenPattern: HiddenPatternSignal | null;
  pulseInstability: number;
...
export function buildRelationalFieldSnapshot(input: BuildRelationalFieldInput): RelationalFieldSnapshot {
  ...
  const entropyScore = input.entropyScore ?? calculateEntropy().entropyScore;
...
export function computeRelationalFieldSnapshot(now = Date.now()): RelationalFieldSnapshot {
  const entropy = calculateEntropy();
  return buildRelationalFieldSnapshot({
    now,
    nodes: useMapState.getState().nodes,
    pulses: usePulseState.getState().logs,
