...
export interface CommandIntervention {
    id: string;
    type: "action";
    label: string;
    subtitle: string;
...
export class CommandOrchestrator {
    ...
    private static cachedInterventions: CommandIntervention[] = [];
    ...
    static async evaluateIntelligence(): Promise<void> {
        ...
        try {
            ...
            console.error("❌ [CommandOrchestrator] Core evaluation failed:", error);
        } finally {
     ...
     */
    private static async synthesizeInterventions(resonance: number, friction: string | null): Promise<CommandIntervention[]> {
        // Fallback for extreme stress if Gemini isn't ready
        const fallbackCrisis: CommandIntervention[] = [
            {
            ...
            },
            {
                id: "ai-lockdown",
                type: "action",
                label: "تجميد النبض (Command Freeze)",
...
[
  {
    ...
    "urgency": "high" | "medium" | "low"
        ...
        try {
            const result = await geminiClient.generateJSON<CommandIntervention[]>(promptContext, "command_intervention");
            if (result && Array.isArray(result) && result.length > 0) {

                return result;
            }
        } catch (e) {
            console.error("[CommandOrchestrator] Gemini failed to synthesize interventions", e);
        }
    ...
    static async dispatchTacticalPreset(presetId: string, message: string): Promise<void> {
        console.log(`[CommandOrchestrator] Dispatching Tactical Preset: ${presetId}`);
    ...
    static async executeIntervention(actionId: string): Promise<void> {
        console.log(`🚀 [CommandOrchestrator] Executing Intervention: ${actionId}`);
        
        // Build payload
        const decisionPayload = { actionId };
        const decisionReasoning = `Command Admin طلب تنفيذ تكتيك آلي: ${actionId}`;
        
        const decisionRes = await decisionEngine.evaluate({
            type: "command_intervention_deployed",
        ...
        if (decisionRes.requiresApproval) {
            console.warn(`[CommandOrchestrator] Requires Approval for ${actionId}`);
