...
export function derivePressureSentence(
  ...
  if (node?.isEmergency || scenarioKey === "emergency") {
    ...
    return {
      tone: "danger",
      title: "جملة الحماية السريعة (درع الطوارئ)",
      summary: `لأن ${displayName} يمثل خطراً استنزافياً حقيقياً، الجملة دي بتحط حد قاطع ينهي أي توقعات منه قبل ما يطلب منك حاجة.`,
      sentence,
      reasoning: "بدل ما ترد على فعل هو بيعمله، أنت بتعلن وضعك الحالي 'أنا محتاج هدوء'. ده بيقفل الباب قدام أي محاولة لاستدراجك في حوار متعب.",
      sourceLabel: "بروتوكول الطوارئ",
      copyText: sentence
    };
  }
