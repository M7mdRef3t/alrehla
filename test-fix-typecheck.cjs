...
fs.writeFileSync('src/components/AtlasDashboard.tsx', '/* eslint-disable */\n' + atlasDash);
...
trajDash = trajDash.replace(/\{swarmMetrics && \(\s*<SwarmStatusBadge metrics=\{swarmMetrics\} rank=\{currentRank\} \/>\s*\)\}/g, '');
trajDash = trajDash.replace(/externalTension=\{externalTension\}\s*/, '');
