...
### Performance Note: Journey Tracking Upserts
Journey tracking was optimized (`src/services/journeyTracking.ts`) by batching tracking events and performing single bulk upsert database queries for User Session stitching instead of an N+1 repeated query.
