...
## FlowMindMap Refactoring

- The large monolithic `FlowMindMap.tsx` file was modularized to improve code maintainability and health.
- It is now organized under `src/components/admin/FlowMindMap/` with separated `types.ts`, `constants.ts`, `utils.ts`, `storage.ts`, and `components.tsx`.
- No functionality changes were made during this structural refactor.
