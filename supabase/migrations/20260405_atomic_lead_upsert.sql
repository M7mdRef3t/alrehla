-- Atomic Marketing Lead Ingestion (Dawayir Hardening)
-- This function handles deduplication (Phone-First, then Email) in a single transaction.
...
CREATE OR REPLACE FUNCTION public.upsert_marketing_lead_v2(
    ...
    p_status TEXT,
    p_intent TEXT,
    p_anonymous_id TEXT DEFAULT NULL -- Restored from previous version
)
RETURNS TABLE (
    lead_id UUID,
    is_new BOOLEAN,
...
BEGIN
    ...
    IF p_phone_normalized IS NOT NULL THEN
        ...
        FOR UPDATE SKIP LOCKED; -- Lock the row if it exists, skip if someone else is updating it
        
        -- If it was locked by another process, we might need a small retry or just wait.
        -- Standard FOR UPDATE without SKIP LOCKED is better for consistency here.
    ...
    IF v_existing_id IS NOT NULL THEN
        ...
        SET 
            ...
            utm = marketing_leads.utm || p_utm, -- Merge UTM data
            note = marketing_leads.note || E'\n' || COALESCE(p_note, ''),
            status = COALESCE(p_status, status),
            intent = COALESCE(p_intent, intent),
            anonymous_id = COALESCE(p_anonymous_id, anonymous_id), -- Restore attribution
...
-- Add filtered unique index if it doesn't exist to back up the RPC logic at the DB level
CREATE UNIQUE INDEX IF NOT EXISTS marketing_leads_phone_unique_idx 
